...
public abstract class Builder<T> {
     ...
     */
    public static final ArrayList<String> trueWords = new ArrayList<>(Arrays.asList("1", "+", "y", "yes", "t", "true"));
    /**
     * Статический массив для хранения слов которые следует распознавать как значение false
     */
    public static final ArrayList<String> falseWords = new ArrayList<>(Arrays.asList("0", "-", "n", "no", "not", "f", "false"));

    public Builder(Printable consoleOutput, InputReader consoleInput) {
        this.consoleOutput = consoleOutput;
        this.consoleInput = consoleInput; // ConsoleInput.isFileMode() ? new RunnableScriptsManager() : consoleInput;
    }
